...
public class AsyncTask_login extends AsyncTask<String, Void, String> {
    ...
    AsyncTask_login(Context context) {
...
//            String Auto_login_userID = Auto_login.getString("userID_putExtra", "");
            ...
            // 자동 로그인 정보 쉐어드에 저장
            SharedPreferences Auto_login = context.getSharedPreferences("Auto_login", context.MODE_PRIVATE);
            SharedPreferences.Editor Auto_login_edit = Auto_login.edit();
            Auto_login_edit.putString("user_no", user_no).apply();
            Auto_login_edit.putString("login_path", login_path).apply();
            Auto_login_edit.putString("email", user_email).apply();
            Auto_login_edit.putString("pw", pw_str).apply();
            ...
            Toast.makeText(context, "아이디, 혹은 비밀번호가\n올바르지 않습니다", Toast.LENGTH_SHORT).show();
        }
    }
}
