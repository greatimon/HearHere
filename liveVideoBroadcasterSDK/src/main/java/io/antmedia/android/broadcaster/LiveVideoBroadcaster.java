...
public class LiveVideoBroadcaster extends Service implements ILiveVideoBroadcaster, CameraHandler.ICameraViewer, SurfaceTexture.OnFrameAvailableListener {
    ...
    private static final String TAG = LiveVideoBroadcaster.class.getSimpleName(); // 액티비티 이름
    private volatile static CameraProxy sCameraProxy; // 카메라 관련 - 패스
    ...
    public boolean startBroadcasting(String rtmpUrl) {
        ...
        try {
            ...
            if (result) {
                ...
                audioThread.start();
                Log.d("streaming", "audioThread.start()??");
                isRecording = true;
